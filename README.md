...
> **v0.4.0 (2026-08-29): tablet-only window-manage popup — 关闭 / 移动窗口到工作区(1–10) / 布局切换(dwindle⇄scrolling, 持久化同 SUPER+L)。移除最大/最小化(平板无意义); Hyprland 触摸不聚焦→动作目标=最后触摸的窗口(可见窗口才可操作)**
...
| 13 | **Tablet window manage (v0.4: close / move ws / layout)** | ✅ implemented live. **Tablet-only** bar entry (laptop mode = clean bar). Close ✕ + move-to-workspace grid (1–10, current highlighted) + Dwindle/Scrolling toggle (persisted to `~/.local/state/omarchy/workspace-layouts/`, same as SUPER+L). Max/min/restore REMOVED (pointless on a tiled touchscreen). Targeting: Hyprland does NOT focus on touch tap, so actions hit the window under the **last touch**; hidden/scratchpad windows are never targeted (an invisible target made actions look dead — root cause of the 0.3.0 “no effect” report). All verified live incl. notify feedback. Needs physical tap pass |
...
| `~/.local/bin/omarchy-window` + `scripts/omarchy-window` | v0.4 — `resolve|close|move <ws>|layout <dwindle|scrolling|toggle>` via 0.56 Lua API; only VISIBLE windows targeted; layout persisted like omarchy's SUPER+L; notify feedback | (repo archive) |
...
| `~/.config/omarchy/plugins/maxt.tablet-experience/BarWidget.qml` | v0.4 rewrite — widget + popup visible ONLY in tablet mode; ✕ close / ➜ move-to-workspace grid / Dwindle·Scrolling buttons (active-ws polled) / Laptop·Tablet / rotation; IPC target `maxt.tablet-window` | (repo archive) |
...
**Done this round (2026-08-29, v0.4.0):** window-manage popup reworked to the tablet reality: **close ✕ + move-to-workspace (1–10 grid) + Dwindle/Scrolling layout toggle** (persisted exactly like omarchy's SUPER+L), shown **only in tablet mode**. Maximize/minimize/restore removed (a tiled WM makes them feel like no-ops: a single window is already full-width; “minimized” windows keep focus, so later actions invisibly operate on the hidden window — the root cause of the 0.3.0 “nothing works” report). Fixes: targets must be VISIBLE (never special/scratchpad), move/layout get Omarchy desktop notifications. Stray user windows that ended up in scratchpad during earlier testing were restored to ws1.
