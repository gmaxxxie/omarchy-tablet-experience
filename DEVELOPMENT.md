...
| 15 | **Virtual-keyboard number row for Chinese candidates (v1.7) + arrows (v1.11)** | ✅ done. squeekboard reads its layout from GNOME input-sources GSettings (`sources` empty on Omarchy → layout name ""), so install.sh now sets `sources=[('xkb','us')]` and ships custom `config/squeekboard/{us,us_wide}.yaml` with a **full number row** on the base/upper views → fcitx5/Rime picks candidates with 1-9. **v1.11: a single arrow row (← → ↑ ↓, keysym Left/Right/Up/Down, unicode labels) added to base/upper** so the caret can be moved from the keyboard too. Verified: squeekboard logs `Loaded layout .../us_wide.yaml`, `squeekboard-test-layout` OK. NOTE: squeekboard fixes the panel height (~213px), so extra rows squeeze the keys slightly (6 rows ≈ 35px/row vs 42px for 5) — acceptable, keys stay touch-sized |
