...
| 14 | **Voice input (v1.5, voxtype hold-to-talk)** | ✅ implemented. Tablet bar **mic icon** → bottom-centered hold-to-talk button (MultiPointTouchArea, mouse+touch); press = `voxtype record start` (SIGUSR1), release = `record stop` (SIGUSR2 → transcribe + type at cursor via wtype, CJK OK). Live state mirrors `$XDG_RUNTIME_DIR/voxtype/state` (idle/recording/transcribing/…) so visuals agree with F9 / SUPER+CTRL+X. Mic icon again (or leaving tablet mode) closes the overlay and never strands a recording. IPC: `voiceInputToggle|Show|Hide`. voxtype-bin 1.0.0 already deployed on this machine (daemon + sensevoice zh + wtype) |
