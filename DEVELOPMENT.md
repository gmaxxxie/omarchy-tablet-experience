...
Implemented:
1. **Lock screen** — `config/hypr/tablet-experience.lua` gains `"above_lock 2, match:namespace wvkbd"`. Service.qml: polls `omarchy-shell lock status` (NOT `-q` — that suppresses ALL output; JSON `locked` = lockRequested||sessionLocked||secure) every 1.5 s; locked + tablet → `texp-vk show`; unlocked → hide; **pre-starts the keyboard hidden on tablet entry** (`texp-vk start-hidden` + settleTimer) so the lock can raise it by signal — no new Wayland client has to connect under a lock. Laptop mode never shows it; leaving tablet puts a lock-shown keyboard down. IPC `getState` now reports `locked`/`lockVkShown`.
...
Verified on this machine: patched wvkbd-deskintl builds + the ▼ bytes are in the binary; `texp-vk start-hidden/show/hide` all live-tested (layer appears/disappears, state file tracks); shell restarted with the new Service.qml — IPC getState responds (locked/lockVkShown), no QML errors; `hyprctl reload` + empty configerrors; texp-sddm-vk both branches tested with a fake wvkbd (detached+touchscreen → launches, attached → no-op). **Not yet verified live: the actual lock-screen render (needs a real lock — do it with fingerprint/password ready) and the SDDM greeter (needs a reboot / autologin-off).**
