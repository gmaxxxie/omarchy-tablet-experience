...
| 14 | **Voice input (v1.5, voxtype hold-to-talk + ⏎ v1.6 + Delete/Clear v1.10)** | ✅ implemented. Tablet bar **mic icon** → bottom-right hold-to-talk button (MultiPointTouchArea, mouse+touch); press = `voxtype record start` (SIGUSR1), release = `record stop` (SIGUSR2 → transcribe + type at cursor via wtype, CJK OK). **English action row** below (v1.10.1: neat equal-width buttons, English hint): Delete (BackSpace), Clear (select-all + delete), Enter (Return) to fix/submit the dictated line. Live state mirrors `$XDG_RUNTIME_DIR/voxtype/state` (idle/recording/transcribing/…) so visuals agree with F9 / SUPER+CTRL+X. Mic icon again (or leaving tablet mode) closes the overlay and never strands a recording. Position is **relative** (right+bottom anchored, 28px margins) — verified 28px from the right in both landscape & portrait; overlay 240x230 with the 3-button row. IPC: `voiceInputToggle|Show|Hide`. voxtype-bin 1.0.0 already deployed on this machine (daemon + sensevoice zh + wtype) |
| 15 | **Virtual-keyboard number row for Chinese candidates (v1.7)** | ✅ done. squeekboard reads its layout from GNOME input-sources GSettings (`sources` empty on Omarchy → layout name ""), so install.sh now sets `sources=[('xkb','us')]` and ships custom `config/squeekboard/{us,us_wide}.yaml` with a **full number row** on the base/upper views → fcitx5/Rime picks candidates with 1-9. Verified: squeekboard logs `Loaded layout .../us_wide.yaml` (file beats builtin), `squeekboard-test-layout` OK, OSK surface 213px = 5 rows. Deployed live on this machine |
