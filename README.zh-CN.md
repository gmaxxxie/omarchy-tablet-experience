...
- **锁屏 + 登录屏虚拟键盘（v1.17，平板模式）** — 平板模式下 Omarchy **锁屏**会出现虚拟键盘用于输密码，解锁后自动收起（Hyprland `above_lock` layer 规则把 wvkbd 渲染到 Quickshell 锁屏表面之上；服务轮询 `omarchy-shell lock status`）。**SDDM 登录屏**同样支持：greeter 自己跑一个 Hyprland，派生合成器配置会在 folio 键盘拆下且存在触摸屏时启动键盘。项目自建的 wvkbd-deskintl 底部右侧带一个可见的 **▼ 收起键**，在锁屏 / 登录 / 日常平板使用中都能把键盘收起来（SDDM 自带的 `InputMethod=qtvirtualkeyboard` 在 Wayland 下是死路）。
