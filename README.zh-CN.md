...
- **语音输入（v1.5，⏎ v1.6，Delete/Clear v1.10，平板模式）** — 顶栏**麦克风图标**打开屏幕底部的一个「按住说话」按钮（再点一次图标可关闭）。按住按钮用 **voxtype** 语音识别（本地 ASR，不上云），松开即转写并输入到光标处——`wtype` 完整支持中文。按钮下方有一排英文按钮 **Delete**（退格）、**Clear**（全选+删除）、**Enter**（回车提交）用于修正或发送刚识别的内容。按钮实时显示 录音 / 识别中 状态，与 F9 / SUPER+CTRL+X 热键同步。
...
| 语音输入（按住说话） | **麦克风 bar 按钮（仅平板模式）** → 底部按钮：按住录音，松开转写；**Delete / Clear / Enter** 修正或提交；再点麦克风图标关闭 |
