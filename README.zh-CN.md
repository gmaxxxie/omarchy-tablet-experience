...
- **锁屏 + 登录屏虚拟键盘（v1.17，两种模式都支持）** — Omarchy **锁屏**现在同时提供两种方式：指纹，以及**密码框右侧的「键盘」按钮**（点它调出屏幕键盘用触屏输密码；通过 Hyprland `above_lock` 层规则渲染到锁屏之上；锁定自动弹出，平板/笔记本模式都有；用键盘自带的 **▼ 收起键**收起，解锁自动隐藏）。顶栏键盘图标也改为两种模式都显示。**SDDM 登录屏**同样支持：greeter 自己跑一个 Hyprland，派生合成器配置会在 folio 键盘拆下且存在触摸屏时启动键盘。项目自建的 wvkbd-deskintl 底部右侧带一个可见的 **▼ 收起键**，在锁屏 / 登录 / 日常使用中都能把键盘收起来（SDDM 自带的 `InputMethod=qtvirtualkeyboard` 在 Wayland 下是死路）。
