...
- **平板简化顶栏（v1.2）** — LAPTOP 模式保留默认完整图标；TABLET 模式只留必要项（菜单 · 工作区 · 时钟 · 托盘 · 网络 · 音量 · 电源 · 本插件），其余（天气、状态指示灯、蓝牙、tailscale、显示器、更新、场景切换、键盘布局等）收进 **⋮ 溢出弹层**：栏上保持两个核心快捷按钮（输入法 EN⇄中 + 虚拟键盘），弹层列出所有被收纳的图标（点一下放回原位）以及窗口管理、旋转。精简前的原始布局保存在 shell.json 里（`bar.layoutSnapshot`，bar 不渲染该键），切回 laptop 时原样恢复，重启/崩溃都不丢。
