...
- **旋转** — 手动四向循环（`SUPER+SHIFT+R`）、传感器自动跟随（iio-sensor-proxy）、或平板弹层里的 ⟲/⟳ 90° 步进按钮。**切换进 tablet 模式时屏幕不会再自动旋转**（v1.4）：保持当前角度不动，只有显式操作、或 preset=auto 时按传感器识别的方向才旋转。触摸校准随屏幕一起旋转（Hyprland 自身不会做这件事）。**选择 Laptop 模式时屏幕必定回到默认 0° 横向**——即使旋转还在进行中，重置也会排队等它完成。**旋转/重置永远保持屏幕 scale 不变**（v1.19：`hl.monitor()` 不带 scale 字段会把 scale 悄悄重置回默认值，实测 1.25→1.6；texp-rotate 现在总是把当前 scale 透传，进 tablet 时快照 laptop 的 scale，切回时原样恢复）。
- **虚拟键盘（v1.12：wvkbd-deskintl）** — `SUPER+U`、顶部栏键盘按钮（仅平板模式，高亮表示可见）或三指轻点（`texp-touch`）。自 v1.12 起虚拟键盘改用 **wvkbd-deskintl**（wlroots 原生），带 **Ctrl / Super / Alt / Shift / AltGr**、**F1-F12**、**数字行**（中文候选）和**方向键**，按键按平板加大——Omarchy / AI 终端需要的键盘快捷键都能在触屏上用了（squeekboard 带数字行保留为备用）。**屏幕底部上滑手势自 v1.2.1 起默认关闭**；想恢复就在 `~/.config/hypr/autostart.lua` 加一行 `o.launch_on_start("texp-vk daemon")`。`install.sh` 会为用户授予 evdev 访问权限（udev `uaccess` 规则 + `input` 用户组），手势守护进程重启后仍可正常读取触摸屏。
- **锁屏 + 登录屏虚拟键盘（v1.17，两种模式都支持）** — Omarchy **锁屏**现在同时提供两种方式：指纹，以及**密码框右侧的「键盘」按钮**（点它调出屏幕键盘用触屏输密码；通过 Hyprland `above_lock` 层规则渲染到锁屏之上；锁定自动弹出，平板/笔记本模式都有；用键盘自带的 **▼ 收起键**收起，解锁自动隐藏）。**顶栏键盘图标仍仅平板模式显示**（v1.18：笔记本模式有物理键盘，不做顶部栏入口）。**SDDM 登录屏**同样支持：greeter 自己跑一个 Hyprland，派生合成器配置会在 folio 键盘拆下且存在触摸屏时启动键盘。项目自建的 wvkbd-deskintl 底部右侧带一个可见的 **▼ 收起键**，在锁屏 / 登录 / 日常使用中都能把键盘收起来（SDDM 自带的 `InputMethod=qtvirtualkeyboard` 在 Wayland 下是死路）。
- **平板简化顶栏（v1.2，v1.3 增强）** — LAPTOP 模式保留默认完整图标；TABLET 模式只留必要项（菜单 · 工作区 · 时钟 · 托盘 · 网络 · 音量 · 电源 · 本插件），其余（天气、状态指示灯、蓝牙、tailscale、显示器、更新、场景切换、键盘布局等）收进 **⋮ 溢出弹层**：栏上保持两个核心快捷按钮（输入法 EN⇄中 + 虚拟键盘）；弹层里每个图标是 **on/off 开关**（✓=在栏上，点一下隐藏；空=隐藏，点一下显示），还有 **Hide all / Show all** 一键整体切换。**配置弹层内容超高时可滚动**（v1.19："Extra bar icons" 列表太长曾溢出弹层框，现在封顶高度 + 可滚动，必要时才出现滚动条）。精简前的原始布局保存在 shell.json 里（`bar.layoutSnapshot`，bar 不渲染该键），切回 laptop 时原样恢复，重启/崩溃都不丢。
- **输入法快速切换** — 顶部栏按钮（**仅平板模式显示**）显示当前 fcitx5 输入法（EN / 中 / …），点一下即切换（`fcitx5-remote -t`）——补上被隐藏的 fcitx5 切换按钮没有的便利。
- **语音输入（v1.5，⏎ v1.6，Delete/Clear v1.10，方向板 v1.11，平板模式）** — 顶栏**麦克风图标**打开屏幕底部的一个「按住说话」按钮（再点一次图标可关闭）。按住按钮用 **voxtype** 语音识别（本地 ASR，不上云），松开即转写并输入到光标处——`wtype` 完整支持中文。按钮下方有一排英文按钮 **Delete**（退格）、**Clear**（全选+删除）、**Enter**（回车提交），屏幕**左侧还有 ↑↓←→ 方向板**（v1.11）移动光标方便修改。语音输入与虚拟键盘**互斥**（v1.11）：打开一个会自动关闭另一个。按钮实时显示 录音 / 识别中 状态，与 F9 / SUPER+CTRL+X 热键同步。
- **顶部栏拍击显隐（平板模式）** — 没有鼠标就没有悬停，所以在 bar 上方放一条全宽的细边缘条：隐藏时轻点顶部 → 显示，再点一次 → 隐藏（驱动 Omarchy 自己的 `bar-off` 开关）。
- **顶部栏拍击显隐（平板模式）** — 没有鼠标就没有悬停，所以在 bar 上方放一条全宽的细边缘条：隐藏时轻点顶部 → 显示，再点一次 → 隐藏（驱动 Omarchy 自己的 `bar-off` 开关）。
- **手写输入（v1.24.0）** — 顶栏**铅笔图标**（或 `SUPER+I`、或 `omarchy-shell maxt.tablet-experience inkInputToggle`）打开一块**手写画布**，用**触控笔或手指**直接写。识别**完全离线**（PP-OCRv6 ONNX，18708 个汉字 + 拉丁字母/数字；约 21 MB 模型，识别时不联网，笔迹不出本机），结果经 `wtype` 打进当前光标处——与语音输入同一套投递方式，终端和各种 toolkit 里中文都能用。面板是 **layer-shell overlay 且 `keyboard_mode=none`**，**不会抢走你正在输入的应用的焦点**。写一个字后停笔：候选出现并自动上屏（面板上的 **自动上屏 / 手动选字** 可切换；手动模式下点候选上屏；自动上屏后点其它候选可直接改选）。**单字 / 整行** 在「单字候选」与「贪婪 CTC 整行解码」（英文单词 / 连笔）之间切换；**清除 / 退格 / 空格 / 回车 / 撤销** 一键可用。面板可用标题处的 **✥ 手柄拖动**，**默认在屏幕右侧、垂直居中**，拖到哪里下次就还在哪里。识别模型可切换：`texp-ink tier tiny|small|medium`，或改 `~/.config/texp-ink/config` 里的 `tier`（该文件还可调 `auto`、`idle_ms`、`min_conf`、`w`、`h`、`stroke_w`）；**实测 `small` 又快又准，默认用它**，`medium` 保留仅供对比。与虚拟键盘、语音输入互斥；进入平板模式时预先在后台加载好（隐藏），首次点击秒开。笔压会调制笔迹粗细。
- **多点触控手势**（`texp-touch`，被动 evdev 监听、不抢占设备）— **双指轻点 = 鼠标右键**（v1.12.2，经 ydotool 注入，呼出上下文菜单 / AI 终端粘贴）· 双指左/右滑：上一个/下一个工作区 · 双指下滑：呼出 Omarchy 菜单 · 单指轻点：聚焦被点窗口（Hyprland 触摸从不聚焦）· **屏保显示时任意触摸即退出屏保**（v1.17.2 — 全屏触摸屏上没有按键/失焦可唤醒，改为直接 SIGTERM 屏保，idle 服务随即唤醒屏幕）。
- **键盘自动模式切换** — 接上键盘 → laptop 模式，拆下 → tablet 模式（USB 存在性检测，默认开启）。
- **平板窗口管理（v1.8）** — 平板模式**独立的窗口 icon**（不再藏在 ⋮ 里）点开即可 关闭 ✕ / 移动到工作区 1–10；⋮ 弹层保留旋转、布局和隐藏的顶栏图标。只瞄准可见窗口，操作有通知反馈。
...
| 语音输入（按住说话） | **麦克风 bar 按钮（仅平板模式）** → 底部按钮：按住录音，松开转写；**Delete / Clear / Enter** 修正或提交；**左侧 ↑↓←→ 方向板**移动光标；再点麦克风图标关闭 |
| 窗口管理（关闭 / 移动） | **窗口 bar 按钮（仅平板模式，v1.8）** → 关闭 ✕ / 移动到工作区 1–10 |
...
| 鼠标右键（上下文菜单/粘贴） | **双指轻点**（v1.12.2，ydotool） |
| 上一/下一工作区 | 双指左/右滑 |
| Omarchy 菜单 | 双指下滑 |
| 退出屏保 | **屏保显示时任意轻点**（v1.17.2 — 全屏触摸屏没有按键/失焦可唤醒） |
...
- `gtk4-layer-shell` — 手写面板 overlay（v1.24.0）
- `python-onnxruntime-cpu`、`python-numpy`、`python-pillow` — 离线手写识别（v1.24.0；`install.sh --no-ink` 可跳过）
